...
- 🎨 **图生图 (Image-to-Image)**：支持上传参考图片，基于参考图生成新图片。 / Supports uploading reference images to generate new images based on them.
- 🔀 **多图融合 (Image Fusion)**：支持多张图片融合，包括风格迁移、换脸、混合等模式。 / Supports fusing multiple images, including style transfer, face swap, and blend modes.
...
```bash
# 基于参考图生成新图片
npx ts-node scripts/main.ts "一只穿着西装的猫" --ref-image=./cat.png --ref-strength=0.7

# 参数说明 / Parameters:
# --ref-image=<path>    参考图片路径 / Reference image path
# --ref-strength=<0-1>  参考强度，值越大越像参考图 / Strength (0=loose, 1=strict)
```
...
```bash
# 融合两张图片
npx ts-node scripts/main.ts "融合风格" --fuse-images=./style.png,./content.png --fuse-mode=style

# 参数说明 / Parameters:
# --fuse-images=<paths>  逗号分隔的图片路径 / Comma-separated image paths
# --fuse-mode=<mode>     融合模式 / Fusion mode:
#   - blend: 混合两张图 (默认/default)
#   - style: 风格迁移 (第一张风格应用到第二张)
#   - face:  换脸 (第一张人脸换到第二张)
```
...
1. **本项目仅供学习研究**：请勿用于违反豆包平台用户协议的商业用途。
2. **账号风险自负**：频繁自动化操作可能导致账号受限，使用者自行承担风险。
3. **非官方工具**：本项目与字节跳动/豆包官方无关，不保证长期可用性。
4. **法律合规**：使用者应确保所在地区/国家允许此类自动化工具的使用。
