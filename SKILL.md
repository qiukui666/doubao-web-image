---
name: "doubao-web"
description: "Use Playwright to host a browser and call Doubao Web's image generation function. Supports text-to-image, image-to-image, and image fusion. Call this skill when the user requests to draw or generate an image using Doubao. (使用 Playwright 托管浏览器的方式，调用豆包 Web 端生图功能。支持文生图、图生图、多图融合。当用户要求使用豆包画图、生成图片时调用此技能。)"
instructions: |
  ...
  5. If the user provides a reference image, use `--ref-image=<path>` for image-to-image generation. (如果用户提供了参考图片，使用 `--ref-image=<path>` 进行图生图。)
  6. If the user wants to fuse multiple images, use `--fuse-images=<paths>` with comma-separated paths. (如果用户要融合多张图片，使用 `--fuse-images=<paths>` 逗号分隔路径。)
  7. By default, execute the command in headless mode in the background. (默认使用后台无头模式执行命令。)
  8. After execution, show the user the path of the generated image or confirm successful generation. (执行完毕后，向用户展示生成的图片路径或确认生成成功。)
---
...
- **Image-to-Image (图生图)**: Upload reference image to generate new image based on it (上传参考图基于其生成新图片)
- **Image Fusion (多图融合)**: Blend multiple images with style transfer, face swap, or blend modes (多张图片融合，支持风格迁移、换脸、混合)
...
# Image-to-Image: Generate based on reference image
# 图生图：基于参考图生成
npx ts-node scripts/main.ts "A cat in a suit" --ref-image="./cat.png" --ref-strength=0.7

# Image Fusion: Blend multiple images
# 多图融合：融合多张图片
npx ts-node scripts/main.ts "Blend style" --fuse-images="./style.png,./content.png" --fuse-mode=style
```
...
| `--ratio=<value>` | Image aspect ratio selection. Supported: `1:1`, `2:3`, `3:4`, `4:3`, `9:16`, `16:9` / 图片比例选择 | |
| `--ref-image=<path>` | Reference image path for image-to-image generation / 图生图的参考图片路径 | |
| `--ref-strength=<0-1>` | Reference strength for image-to-image (0=loose, 1=strict) / 图生图参考强度 | `0.7` |
| `--fuse-images=<paths>` | Comma-separated image paths for fusion / 多图融合的图片路径(逗号分隔) | |
| `--fuse-mode=<mode>` | Fusion mode: `blend`, `style`, `face` / 融合模式：混合、风格迁移、换脸 | `blend` |
